...
output_path = args[4]
sce = args[5]
# snrna_path = "Z:/jxliu/project/ID-GAT/ID/data/heart_development/scRNA-seq-R/scRNA-seq_development_heart_with_meta.h5Seurat"
# spatial_path = "Z:/jxliu/project/ID-GAT/ID/data/heart_development/ST/ST_development_heart_FH5_1000L3_CN20_D2_4.h5Seurat"
# celltype_final = 'cell_type'
# output_path = "Z:/jxliu/project/ID-GAT/ID/results/Others/human_developing_heart"
res_file_name <- tail(strsplit(spatial_path, '/')[[1]], 1)
res_file_name <- strsplit(res_file_name, '.h5seu')[[1]][1]
res_file_name <- paste0(output_path, 
                         '/RCTD_', res_file_name, '_', sce,  '_results.csv')
print(res_file_name)

sc_obj <- LoadH5Seurat(snrna_path)
diff_list <- list()
c = 0
# replace cell type names to avoid mistakes
cell_type_list <- sc_obj@meta.data[,celltype_final]
new_type_list <- gsub('/', '_or_', cell_type_list)
sc_obj@meta.data[,celltype_final] <- new_type_list
...
res_columns <- colnames(norm_weights)
res_columns <- gsub('_or_', '/', res_columns)
colnames(norm_weights) <- res_columns
write.csv(norm_weights, res_file_name)
